...
//	$validasi_existing_pelanggan = mysqli_query($con, "SELECT no_telp FROM customer where no_telp =$no_telp ");

 	$validasi_existing_pelanggan = mysqli_query($con, "SELECT no_telp FROM customer pl JOIN antrian pn ON pl.id_customer = pn.id_customer where no_telp = '$no_telp' AND tanggal_order = '$current_date'");

	$validasi_jumlah_existing_pelanggan = mysqli_num_rows($validasi_existing_pelanggan);

	if($validasi_jumlah_existing_pelanggan >= 1)

	{
									...
									swal({
									  title: "Maaf",
									  text: "Anda telah terdaftar pada antrian hari ini",
									  icon: "warning",
									  button: "Ok!",
									})
									.then((value) => {
